...
class Client:
    ...
    def __deploy_server(self,jar_path) -> None:
        """
        Deploy server to android device
        """
        # if hasattr(sys, '_MEIPASS'):
        #     jar_name = os.path.join(sys._MEIPASS, 'scrcpy-server.jar')
        # else:
        #     jar_name = './scrcpy-server.jar'
        # server_file_path = os.path.join(
        #     os.path.abspath(os.path.dirname(__file__)), jar_name
        # )
        self.device.sync.push(jar_path, f"/data/local/tmp/{jar_path}")
        print(jar_path)
        commands = [
            f"CLASSPATH=/data/local/tmp/{jar_path}",
            "app_process",
            "/",
    ...
    def start(self, jar_path, threaded: bool = False, daemon_threaded: bool = False) -> None:
        """
